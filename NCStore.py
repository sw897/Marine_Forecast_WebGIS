    ...
    def iter_points(self, projection=LatLonProjection):
        ...
        for i in xrange(0, self.n):
            for j in xrange(0, self.n):
                # 取格网中心点
                yield projection.unproject(Point(xmin + new_res * (j + .5), ymax - new_res * (i + .5)))
...
class Extent(object):
    ...
    def iter_tilecoords(self, level, projection):
        ...
        nums = pow(2, level)
        col_min = int(
            (extent.xmin - worldextent.xmin) / worldextent.resolution * nums)
        col_max = int(
            (extent.xmax - worldextent.xmin) / worldextent.resolution * nums)
        row_min = int(
            (worldextent.ymax - extent.ymax) / worldextent.resolution * nums)
        row_max = int(
            (worldextent.ymax - extent.ymin) / worldextent.resolution * nums)
...
class WindSymbol(MarineSymbol):
    ...
    def __init__(self, value, angle):
        ...
        if self.level < 9:
            points = np.array(
                [[0, 4], [0, 0], [0, 0], [1, 0], [1, 0], [2, 0], [0, 1], [1, 1], [1, 1], [2, 1], [0, 2], [1, 2],
                [1, 2], [2, 2], [0, 3], [1, 3], [1, 3], [2, 3]])
            self.points = points[0:2 * (self.level + 1)]
        else:
            self.points = np.array(
                [[0, 4], [0, 0], [0, 0], [2, 2], [0, 2], [2, 2]])
        self.rotate(angle)

    def speed2level(self, speed):
        values = [0.3, 1.6, 3.4, 5.5, 8.0, 10.8,
                  13.9, 17.2, 20.8, 24.5, 28.5, 32.6, 32.6]
    ...
    def draw_agg(self, draw, style):
        ...
        for it in range(len(self.points) / 2):
            draw.line(
                tuple(list(self.points[2 * it]) + list(self.points[2 * it + 1])), pen)
        # draw.flush()
...
class ArrowSymbol(MarineSymbol):
    ...
    def draw_agg(self, draw, style):
        # draw.setantialias(True)
        points = list(self.polygon[0]) + list(self.polygon[1]) + list(
            self.polygon[2]) + list(self.polygon[0])
        pen = aggdraw.Pen(tuple(style.color), style.width)
...
class ColorModelUtility(object):
    ...
    @classmethod
    def rgb2hex(cls, rgb):
        if rgb is None or len(rgb) != 3 or min(rgb) < 0 or max(rgb) > 255:
            return 0xFFFFFF
...
class NCThumbnail(object):
    ...
    def generate(self):
        ...
        model_regions = {"wrf": ["nwp", "ncs", "qdsea"], "swan": ["nwp", "ncs", "qdsea"], "ww3": [
            "global", "nwp"], "pom": ["ncs"], "roms": ["nwp", "ncs", "qdsea"], "fvcomstm": ["bhs"]}
        # type_methods = {}
...
class NCStore(object):
    ...
    def get_colrow(self, latlon):
        if latlon is not None:
            ...
            lat_index = int(
                math.ceil((latlon.lat - self.extent.ymin) / self.resolution))
    ...
    def get_latlon(self, rowcol):
        if rowcol is not None:
            lon = rowcol.col * \
                ...
                self.cols + self.extent.xmin
            lat = rowcol.row * \
                (self.extent.ymax - self.extent.ymin) / \
                self.rows + self.extent.ymin
            return LatLon(lat, lon)
        else:
            return None
    ...
    def get_style(self, variables, time=None, level=None):
        if self.universal_style:
            stylefile = os.path.join(
                self.ncfs, ','.join(variables), 'style.yaml')
    ...
    def get_json_tile_filename(self, tilecoord, variables, time, level, projection):
        ...
        dirname = os.path.join(
            self.ncfs, ','.join(variables), "json_tile", code, "%d_%d" %
                               (time, level), str(tilecoord.z), str(tilecoord.y))
        if not os.path.isdir(dirname):
            os.makedirs(dirname)
    ...
    def get_image_tile_filename(self, tilecoord, variables, time, level, projection, postProcess=None):
        if projection == WebMercatorProjection:
            ...
            code = '4326'
        dirname = os.path.join(
            self.ncfs, ','.join(variables), "image_tile", code, "%d_%d" %
                               (time, level), str(tilecoord.z), str(tilecoord.y))
        if not os.path.isdir(dirname):
            os.makedirs(dirname)
    ...
    def get_scalar_image(self, variables=None, time=0, level=0, projection=LatLonProjection, update=False):
        if variables is None:
            variables = self.default_scalar
        filename = self.get_scalar_image_filename(
            variables, time, level, projection)
        update = update | self.check_cache_valid(filename)
        if update:
    ...
    def get_image_tile(self, tilecoord, variables, time, level, projection, postProcess=None, update=False):
        ...
        filename = self.get_image_tile_filename(
            tilecoord, variables, time, level, projection)
        update = update | self.check_cache_valid(filename)
        if update:
            if isinstance(self, TINStore):
                filename = self.vector_to_nogrid_image_tile(
                    tilecoord, variables, time, level, projection, postProcess)
            else:
                filename = self.vector_to_grid_image_tile(
                    tilecoord, variables, time, level, projection, postProcess)
        return filename
    ...
    def get_json_tile(self, tilecoord, variables, time, level, projection, postProcess=None, update=False):
        if variables is None:
            variables = self.default_variables
        filename = self.get_json_tile_filename(
            tilecoord, variables, time, level, projection)
        update = update | self.check_cache_valid(filename)
        if update:
            if isinstance(self, TINStore):
                json = self.vector_to_nogrid_json_tile(
                    tilecoord, variables, time, level, projection, postProcess)
            else:
                json = self.vector_to_grid_json_tile(
                    tilecoord, variables, time, level, projection, postProcess)
            fp = open(filename, 'w')
            fp.write(json)
            fp.close()
    ...
    def generate_legend(self, variables, time=None, level=None):
        ...
        for i in range(bar_x):
            ...
            draw.line(
                (i + margin_x, 0 + margin_y, i + margin_x, bar_y + margin_y), pen)
        # draw color bar bound
        draw.rectangle(
            (0 + margin_x, 0 + margin_y, bar_x + margin_x, bar_y + margin_y), black_pen)
        # draw scalar marker
        ...
        for v in value_marks:
            ...
            if pos > bar_x:
                pos = bar_x
            draw.line(
                (pos + margin_x, bar_y + margin_y, pos + margin_x, bar_y + len_mark + margin_y), black_pen)
            draw.text(
                (pos + margin_x + font_margin_x, bar_y + len_mark + font_margin_y + margin_y), str(v), font)
        # save
        draw.flush()
        del draw
    ...
    # 输出 vector grid image or geojson tiles 序列, type=image|json, default is
    # image
...
class TINStore(NCStore):
    ...
    def init(self):
        ...
        self.cols = int(
            (self.extent.xmax - self.extent.xmin) / self.resolution)
        self.rows = int(
            (self.extent.ymax - self.extent.ymin) / self.resolution)
        self.set_filter_extent(
            self.extent.xmin, self.extent.ymin, self.extent.xmax, self.extent.ymax)
    ...
    def export_nodes(self, outputFormat='ESRI Shapefile'):
        dirName = self.shpfs
        layerName = 'nodes'
        outputFormats = {'ESRI Shapefile': '.shp', 'GeoJSON':
                         '.geojson', 'JSON': '.json', 'SQLite': '.sqlite'}
        outputfile = os.path.join(
            dirName, layerName + outputFormats[outputFormat])
        ...
        layer = dataSource.CreateLayer(
            layerName, spatialReference, ogr.wkbPoint)
    ...
    def export_elements(self, outputFormat='ESRI Shapefile'):
        ...
        spatialReference.ImportFromProj4(
            '+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs')
        layer = dataSource.CreateLayer(
            layerName, spatialReference, ogr.wkbPolygon)
        ...
        for element in range(self.elements):
            ...
            polygon = geometry.Polygon([(self.getLon(nodes1[element]), self.getLat(nodes1[element])), (
                self.getLon(nodes2[element]), self.getLat(nodes2[element])), (self.getLon(nodes3[element]), self.getLat(nodes3[element]))])
    ...
    def export_element_centers(self, outputFormat='ESRI Shapefile'):
        ...
        for element in range(self.elements):
            feature = ogr.Feature(featureDefinition)
            lat = (self.getLat(nodes[0][element]) + self.getLat(
                nodes[1][element]) + self.getLat(nodes[2][element])) / 3.
            lon = (self.getLon(nodes[0][element]) + self.getLon(
                nodes[1][element]) + self.getLon(nodes[2][element])) / 3.
            point = geometry.Point(lon, lat)
            feature.SetGeometry(ogr.CreateGeometryFromWkb(point.wkb))
            feature.SetField(0, element)
            layer.CreateFeature(feature)
    ...
    def export_elements_gridnc(self):
        ...
        element = gridnc.createVariable(
            'element', 'i4', ('temp', 'lat', 'lon'))
    ...
    def get_triangles(self, tilecoord, handle, projection=LatLonProjection, nogrid=False, buffer=0.):
        ...
        if isinstance(handle, list):
            ...
            if nogrid:
                ...
                # 根据bbox过滤
                if min.lon < self.filter_extent.xmin:
                    min.lon = self.filter_extent.xmin
                if min.lat < self.filter_extent.ymin:
                    min.lat = self.filter_extent.ymin
                if max.lon > self.filter_extent.xmax:
                    max.lon = self.filter_extent.xmax
                if max.lat > self.filter_extent.ymax:
                    max.lat = self.filter_extent.ymax
        ...
        if projection == LatLonProjection:
            ...
            for row in range(height):
                for col in range(width):
                    ...
                    if nele < 0 or np.isnan(float(nele)):
                        continue
                    ...
                    pen = aggdraw.Pen(tuple(rgb), 1)
                    draw.rectangle(
                        (col, height - row, col + 1, height - 1 - row), pen)
            draw.flush()
            del draw
            filename = self.get_scalar_image_filename(
                variables, time, level, projection)
            ...
            ll_max = LatLon(
                ll_min.lat + self.resolution * (self.max_row - self.min_row),
                ll_min.lon + self.resolution * (self.max_col - self.min_col))
            wm_min = WebMercatorProjection.project(ll_min)
            wm_max = WebMercatorProjection.project(ll_max)
            # width = self.cols
            width = self.max_col - self.min_col
            height = int(
                math.ceil((wm_max.y - wm_min.y) / (wm_max.x - wm_min.x) * width))
    ...
    def vector_to_grid_image_tile(self, tilecoord, variables, time, level=0, projection=LatLonProjection, postProcess=None):
        ...
        for triangle in self.get_triangles(tilecoord, handle=handle, projection=projection):
            ...
            if triangle.nele < 0:
                continue
            values = [getattr(self, self.variables[variable])(time=time, element=triangle.nele)
                      for variable in variables]
            if not self.is_valid_values(values):
                continue
        ...
        draw.flush()
        del draw
        tile = self.get_image_tile_filename(
            tilecoord, variables, time, level, projection)
        img.save(tile, "png")
        return tile
    ...
    def get_point_value_json(self, latlon, variables=None, level=None, time=None, postProcess=None):
        if variables is None:
            variables = self.default_variables
        json = '{"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"Point","coordinates":[%f,%f]},"properties": {"value": [' % (
        ...
        for level in levels:
            for time in times:
                for variable in variables:
                    ...
                    if triangle.nele < 0:
                        continue
                    value = getattr(self, self.variables[variable])(
                        time=time, element=triangle.nele)
                    json += '%.6f' % value + ','
        if json[-1] == ',':
            json = json[:-1]
        json += ']}}'
...
class FVCOMTIDStore(TINStore):
    ...
    def __init__(self, date, region='DLW'):
        ...
        try:
            ...
            regiondir = region
            subnames = [
                'FVCOM_crt', regiondir, self.date + '0000UTC', '072', '1hr']
            filename = '_'.join(subnames)
            self.ncfs = os.path.join(os.environ['NC_PATH'], subdir, filename)
            self.shpfs = os.path.join(os.environ['NC_PATH'], subdir, region)
            self.init()
...
class GridStore(NCStore):
    ...
    def get_variable(self, variable, rowcol, time, level):
        ...
        if rowcol is not None:
            ...
            if self.levels > 0:
                value = netcdf.variables[variable][
                    time][level][rowcol.row][rowcol.col]
            else:
                value = netcdf.variables[variable][
                    time][rowcol.row][rowcol.col]
            value = float(value)
        ...
        for level in levels:
            for time in times:
                if isinstance(variables, list):
                    ...
                    values = self.get_value_direct(
                        variables, None, time, level)
                    # values = values[:self.rows, :self.cols]
                    values = values[
                        self.min_row:self.max_row, self.min_col:self.max_col]
                if stack is None:
    ...
    def vector_to_grid_json_tile(self, tilecoord, variables, time, level, projection, postProcess=None):
        ...
        json = '{"type":"FeatureCollection","features":['
        for latlon in tilecoord.iter_points(projection):
            values = [self.get_value(variable, latlon, time, level)
                      for variable in variables]
            if not self.is_valid_values(values):
                continue
            if postProcess is not None:
...
class WRFStore(GridStore):
    ...
    def __init__(self, date, region='NWP'):
        self.no_data = 1e+30
        self.dimensions = {
            'lon': 'longitude', 'lat': 'latitude', 'time': 'times', 'level': None}
...
class SWANStore(GridStore):
    ...
    def __init__(self, date, region='NWP'):
        ...
        self.dimensions = {
            'lon': 'longitude', 'lat': 'latitude', 'time': 'time', 'level': None}
        ...
        try:
            ...
            filename = '_'.join(subnames)
            self.ncfs = os.path.join(
                os.environ['NC_PATH'], subdir, dirmap[region], filename)
            self.init()
        except Exception, e:
            print e.__str__()
...
class POMStore(GridStore):
    ...
    def __init__(self, date, region='NCS'):
        ...
        try:
            ...
            if region == 'ECS':
                subnames = [
                    'POM_crt', regiondir, self.date + '0000BJS', '072', '1hr']
            elif region == 'BH':
                subnames = [
                    'POM_crt', regiondir, self.date + '0000BJS', 'a24', '1hr']
            else:
                subnames = [
                    'POM_cut', regiondir, self.date + '00BJS', '072', '1hr']
            filename = '_'.join(subnames)
            self.ncfs = os.path.join(os.environ['NC_PATH'], subdir, filename)
            self.init()
        except Exception, e:
...
class ROMSStore(GridStore):
    ...
    def __init__(self, date, region='NWP'):
        self.no_data = 1e+37
        self.dimensions = {
            'lon': 'xi_v', 'lat': 'eta_u', 'time': 'ocean_time', 'level': 's_rho'}
