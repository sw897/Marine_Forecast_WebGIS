...
L.TileLayer.GeoJSON = L.TileLayer.Ajax.extend({
    ...
    // Recurse LayerGroups and call func() on L.Marker layer instances
    _recurseLayerUntilMarker: function (func, layer) {
        if (layer instanceof L.Marker) {
            func(layer);
        }
        else if (layer instanceof L.LayerGroup) {
            // Recurse each child layer
            layer.getLayers().forEach(this._recurseLayerUntilMarker.bind(this, func), this);
        }
    },
...
L.tileLayer.geojson = function (url, options, geojsonOptions) {
    return new L.TileLayer.GeoJSON(url, options, geojsonOptions);
};
